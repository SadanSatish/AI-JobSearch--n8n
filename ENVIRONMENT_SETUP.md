...
3. **Populate Values**: Open `.env` and replace all dummy placeholders with your actual credentials.
   - **Database**: Get your `DATABASE_URL` from Neon. **CRITICAL: Ensure you are using the Direct Connection, NOT the Pooled Connection. The host must NOT contain `-pooler`.** Ensure it includes `?sslmode=require` at the end.
